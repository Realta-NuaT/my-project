...
import {apiAuthAskCode, apiUserDetails, apiUserDetailSave, apiUserModifyEmail} from "@/net/api/user";
...
const validateUsername = (_, value, callback) => {
  if(value === ''){
    ...
    callback(new Error('用户名不能包含特殊字符,只能包含中/英文'))
  }else{
    callback()
  }
}
...
function askCode() {
  emailFormRef.value.validate((valid) => {
    apiAuthAskCode(emailForm.email, 'modify', coldTime, timer, isEmailValid)
  })
}
...
const rules = {
  username: [
    ...
    {min:2, max:10, message:'用户名长度必须在2-10位之间', trigger:['blur','change'] }
  ],
  phone: [
    {validator: validatePhone,trigger: ['blur','change']},
    {min: 2,max:11, message:'手机号长度必须小于等于11位', trigger:['blur','change'] }
  ],
  email:[
...
<template>
  <div style="display: flex;max-width: 950px;margin: auto">
    <div class="setting-left">
      <card :icon="User" tittle="账号信息设置" desc="在这里编辑您的个人信息,您可以在隐私设置中选择是否展示这些信息"
      ...
      <card style="margin-top: 10px" :icon="Message" tittle="电子邮件设置" desc="您可以在这里修改默认绑定的电子邮件地址">
